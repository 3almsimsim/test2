...
// const form_send = document.querySelector('.form');
// test
...
class Products {
  async getProducts() {
    try {
      let result = await fetch("../../jsonFolder/pen.json");
...
class UI {
  displayProducts(products) {
    ...
    products.forEach((product) => {
      // products = products.slice(product,17);
      // product = products[Math.floor(Math.random() * products.length)];

      //         products.push(product);

      if (product.section === "pen") {
        //
        result += `
  ...
  addCartItem(item) {
    ...
    <div class="head_img"><img src=${item.img} alt=""></div>
        ...
        <h5 id="item_amount">${item.amount}</h5>
        <i class="fas fa-chevron-right" data-id=${item.id}></i>
        </div>
        <div class="head_remove cart-remove">
  ...
  clearCart() {
    ...
    buttons.forEach((button) => {
      let id = button.dataset.id;
      // console.log(id);
      let inCart = cart.find((item) => item.id !== null);
      if (inCart) {
        // button.innerText = "In Cart";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  //   "window.location.reload();"
  //   window.setInterval("element()", 3000);
  //     counter = counter + 1;
...
//             function refreshDiv(){
//                 counter = counter + 1;
//                 document.getElementById("theCart").innerHTML = "Testing " + counter;
//             }
// display products

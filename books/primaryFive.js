...
class Products {
  async getProducts() {
    try {
      let result = await fetch("../jsonFolder/primaryFive.json");
...
class UI {
    ...
    products.forEach(product => {
      if(product.section === "primary_five"){
        result += `
    ...
    `;
    fiveDOM.innerHTML = result;
  }else{
   let result = "";

    products.forEach(product => {
      if(product.section === "primary_six"){
        result += `
        <div class="week-prod product-box">
        <a href=""><img src=${product.img} alt="" class="product-img"></a>
        <a href="#" class="new-p-link">${product.desc}</a>
